...
public class AdminService {
    ...
    public List<ActivityChartPoint> getActivityChart(int days) {
        try {
            ...
            List<Object[]> rows = activityLogRepository.countByActionGroupedByDate("GENERATE_COURSE", since);
            return rows.stream()
    ...
    public List<CategoryDistributionPoint> getCategoryDistribution() {
        try {
            return courseRepository.countGroupedByCategory().stream()
                    .map(r -> new CategoryDistributionPoint((String) r[0], ((Number) r[1]).longValue()))
                    .collect(Collectors.toList());
        } catch (Exception e) {
            ...
            return logs.stream()
                    .map(log -> {
                        String name = (log.getUser() != null)
                                ...
                                : "Unknown";
                        return new RecentActivityEntry(name, log.getAction().name(), log.getTimestamp().format(fmt));
                    })
                    .collect(Collectors.toList());
        } catch (Exception e) {
    ...
    public StudentDetailResponse getStudentDetail(Long studentId) {
        ...
        List<StudentCourseItem> courseItems = courses.stream().map(course -> {
            ...
            int completed = lessonIds.isEmpty() ? 0
                    : (int) lessonProgressRepository.findByStudentIdAndLessonIdIn(studentId, lessonIds)
                            .stream().filter(lp -> Boolean.TRUE.equals(lp.getIsCompleted())).count();
        ...
        List<StudentCertificateItem> certItems = certificateRepository.findAllByStudentId(studentId)
                .stream().map(cert -> {
                    ...
                    return (StudentCertificateItem) StudentCertificateItem.builder()
                            ...
                            .score(cert.getScore())
                            .issuedAt(cert.getIssuedAt() != null ? cert.getIssuedAt().toLocalDate().toString() : null)
        ...
        return examAttemptRepository.findByStudentId(studentId).stream()
                .map(attempt -> {
                    ...
                    return StudentExamAttemptItem.builder()
                            ...
                            .submittedAt(attempt.getSubmittedAt() != null ? attempt.getSubmittedAt().format(fmt) : null)
                            .build();
                }).collect(Collectors.toList());
    }
    ...
    public List<AdminCertificateItem> getAllCertificates() {
        return certificateRepository.findAll().stream()
                .sorted(Comparator.comparing(Certificate::getIssuedAt, Comparator.nullsLast(Comparator.reverseOrder())))
                .map(cert -> {
                    ...
                    if (courseOpt.isPresent()) {
                        ...
                        category = courseOpt.get().getCategory() != null ? courseOpt.get().getCategory() : "";
                    }
    ...
    public List<AdminExamAttemptItem> getAllExamAttempts() {
        ...
        return examAttemptRepository.findAll().stream()
                ...
                .map(attempt -> {
                    ...
                    int suspiciousCount = suspiciousActivityRepository.totalCountForAttempt(attempt.getId());
    ...
    private int[] computeEngagement(Long studentId) {
        ...
        int score = (lessons * 10) + (quizzesPassed * 15) + (examsPassed * 25) + Math.min(flashcardsReviewed, 20);
