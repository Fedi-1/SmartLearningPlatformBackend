...
public class DashboardService {
    ...
    public DashboardResponse getDashboardData(Long studentId) {
        ...
        // ── 1. Courses ────────────────────────────────────────────────────────
        List<Course> courses = courseRepository.findByStudentIdAndIsDeletedFalse(studentId);
        ...
        for (Course course : courses) {
            ...
            if (exam.isPresent()) {
                examPassed = examAttemptRepository
                        .existsByStudentIdAndExamIdAndIsPassed(studentId, exam.get().getId(), true);
            }
        ...
        List<ActivityDto> recentActivity = activityLogRepository
                ...
                .map(log -> ActivityDto.builder()
                        ...
                        .description(log.getEntityType() != null
                                ? log.getAction().name().replace("_", " ") + " — " + log.getEntityType()
