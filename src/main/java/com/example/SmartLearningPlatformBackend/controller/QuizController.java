...
public class QuizController {
        ...
        /**
         * POST /api/quiz-attempts/{attemptId}/abandon
         * Marks the attempt as ABANDONED (score=0, isPassed=false). No lesson unlock.
         */
        @PostMapping("/api/quiz-attempts/{attemptId}/abandon")
        public ResponseEntity<QuizAttemptResponse> abandonAttempt(
                        @PathVariable Long attemptId,
                        @AuthenticationPrincipal UserDetailsImpl principal) {

                return ResponseEntity.ok(
                                quizService.abandonAttempt(attemptId, principal.getUser().getId()));
        }
